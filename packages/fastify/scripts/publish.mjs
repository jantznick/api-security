#!/usr/bin/env node
/**
 * Publish @apiglimpse/fastify to npm.
 *
...
if (pkg.name !== '@apiglimpse/fastify') {
  fail(`expected package name @apiglimpse/fastify, got ${pkg.name}`);
}
...
if (publishOk) {
  if (dryRun) {
    ...
    console.log('Verify: npm view @apiglimpse/fastify version');
  }
}
