#!/usr/bin/env node
/**
 * Publish @apiglimpse/middleware to npm.
 *
 ...
 *
 * Do not use npm prepublishOnly for the swap — that can leave
 * package.json in a bad state if publish fails mid-way.
 *
 * Usage (from packages/middleware):
 *   npm run publish:npm
...
if (pkg.name !== '@apiglimpse/middleware') {
  fail(`expected package name @apiglimpse/middleware, got ${pkg.name}`);
}
...
if (publishOk) {
  if (dryRun) {
    ...
    console.log('Verify: npm view @apiglimpse/middleware version');
  }
}
