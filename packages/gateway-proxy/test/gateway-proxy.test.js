...
describe('createGatewayProxy integration', () => {
  it('forwards HTTP and POSTs /v1/samples', async () => {
    ...
    try {
      ...
      // Wait until the collector receives at least one sample (async flush)
      const deadline = Date.now() + 3000;
      while (samplesReceived.length < 1 && Date.now() < deadline) {
      ...
      if (collector) await new Promise((r) => collector.close(r));
